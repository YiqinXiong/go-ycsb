...
func getTableOptions(p *properties.Properties) (*gorocksdb.BlockBasedTableOptions, bool) {
	...
	if b := p.GetString(rocksdbFilterPolicy, "rocksdb.BuiltinBloomFilter"); len(b) > 0 {
		if b == "rocksdb.BuiltinBloomFilter" {
			...
			tblOpts.SetFilterPolicy(gorocksdb.NewBloomFilterFull(BitsPerKey))
		}
	}
...
func getOptions(p *properties.Properties) *gorocksdb.Options {
	...
	opts.SetMaxSubcompactions(uint32(p.GetUint(rocksdbMaxSubcompactions, 1)))
...
func (db *rocksDB) Close() error {
	...
	if manualCompact == true {
		...
		fmt.Println("Keep compaction before DB closed")
		start := time.Now()
		opts := db.db.Opts()
		...
		fmt.Printf("Keep Compaction finished, takes %s\n", time.Since(start))
	}
	db.db.Close()
	return nil
}
...
func (db *rocksDB) GetStatisticsString() string {
	...
	intervalCompactionStr := fmt.Sprintf("Interval compaction: %.2f GB write, %.2f MB/s write, %.2f GB read, %.2f MB/s read\n",
		float64(st.interval_compact_write_bytes)/kGB, float64(st.interval_compact_write_bytes)/kMB/intervalElapsed,
		float64(st.interval_compact_read_bytes)/kGB, float64(st.interval_compact_read_bytes)/kMB/intervalElapsed)
